...
' 描述: 数据质检场景包：支持内置预设/上次/命名规则复用；按必填、主键唯一、电话、日期、错误值等规则预览问题清单，确认后只写问题明细与质检报告，不改源表
...
Function HostExcelValidateReportPack(appObj)
    ...
    If StrComp(presetChoice, "上次", vbTextCompare) = 0 Or LCase(presetChoice) = "last" Then
        ruleText = ReadTextFile(RuleFilePath("validate", "last"))
        If Len(Trim(ruleText)) = 0 Then
            HostExcelValidateReportPack = FailureJson("E_RULE_LOAD", "未找到可用的上次质检规则")
            Exit Function
        End If
        requiredText = ExtractRuleValue(ruleText, "required")
        keyUniqueText = ExtractRuleValue(ruleText, "keyUnique")
        phoneText = ExtractRuleValue(ruleText, "phone")
        dateText = ExtractRuleValue(ruleText, "date")
        errorText = ExtractRuleValue(ruleText, "error")
        If Len(errorText) = 0 Then errorText = "是"
        ruleNameUsed = "上次"
        ruleLoaded = True
    ElseIf StrComp(presetChoice, "命名规则", vbTextCompare) = 0 Or StrComp(presetChoice, "命名", vbTextCompare) = 0 Then
        Dim named
        named = PromptNamedRuleName("validate")
        If Len(named) = 0 Then
            HostExcelValidateReportPack = FailureJson("E_RULE_NAME", "未提供命名规则名称")
            Exit Function
        End If
        ruleText = ReadTextFile(RuleFilePath("validate", named))
        If Len(Trim(ruleText)) = 0 Then
            HostExcelValidateReportPack = FailureJson("E_RULE_LOAD", "未找到命名质检规则：" & named)
            Exit Function
        End If
        requiredText = ExtractRuleValue(ruleText, "required")
        keyUniqueText = ExtractRuleValue(ruleText, "keyUnique")
    ...
    ElseIf StrComp(presetChoice, "手动", vbTextCompare) = 0 Or LCase(presetChoice) = "manual" Then
        requiredText = Trim(SafePrompt("必填字段（逗号分隔，可空）（表头：" & BuildHeaderHint(sourceRange) & "）", CStr(sourceRange.Cells(1, 1).Text)))
        keyUniqueText = Trim(SafePrompt("主键唯一字段（逗号分隔，可空）（表头：" & BuildHeaderHint(sourceRange) & "）", ""))
        phoneText = Trim(SafePrompt("电话校验字段（逗号分隔，可空）（表头：" & BuildHeaderHint(sourceRange) & "）", ""))
        dateText = Trim(SafePrompt("日期校验字段（逗号分隔，可空）（表头：" & BuildHeaderHint(sourceRange) & "）", ""))
        errorText = Trim(SafePrompt("是否检查错误值（是/否，默认是）", "是"))
        ruleNameUsed = "手动输入"
    Else
        Dim builtinName
        builtinName = Trim(SafePrompt("内置预设：1必填+主键质检 / 2联系方式质检 / 3日期必填质检（默认 1）", "1"))
        If builtinName = "2" Or InStr(1, builtinName, "联系", vbTextCompare) > 0 Or InStr(1, builtinName, "电话", vbTextCompare) > 0 Then
            ruleNameUsed = "联系方式质检"
            requiredText = GuessHeaderByKeywords(sourceRange, "姓名,客户,联系人")
            keyUniqueText = ""
            phoneText = GuessHeaderByKeywords(sourceRange, "电话,手机,联系方式,Mobile,Phone")
            dateText = ""
            errorText = "是"
        ElseIf builtinName = "3" Or InStr(1, builtinName, "日期", vbTextCompare) > 0 Then
            ruleNameUsed = "日期必填质检"
            requiredText = JoinNonEmpty(",", GuessHeaderByKeywords(sourceRange, "姓名,客户,项目,标题"), GuessHeaderByKeywords(sourceRange, "日期,截止日期,到期日,业务日期"))
            keyUniqueText = ""
            phoneText = ""
            dateText = GuessHeaderByKeywords(sourceRange, "日期,截止日期,到期日,业务日期,完成日期")
            errorText = "是"
        Else
            ruleNameUsed = "必填+主键质检"
            requiredText = JoinNonEmpty(",", GuessHeaderByKeywords(sourceRange, "姓名,客户,项目,标题,名称"), GuessHeaderByKeywords(sourceRange, "状态,负责人,日期"))
            If Len(requiredText) = 0 Then requiredText = CStr(sourceRange.Cells(1, 1).Text)
            keyUniqueText = GuessHeaderByKeywords(sourceRange, "编号,单号,ID,编码,工号,单据号,主键")
            phoneText = ""
            dateText = ""
            errorText = "是"
        End If
        Dim overrideAns
        overrideAns = Trim(SafePrompt("将使用预设【" & ruleNameUsed & "】必填=" & requiredText & "；主键=" & keyUniqueText & "；电话=" & phoneText & "；日期=" & dateText & "。回车沿用，或输入“手动”改写", ""))
        If StrComp(overrideAns, "手动", vbTextCompare) = 0 Then
            requiredText = Trim(SafePrompt("必填字段（逗号分隔，可空）（表头：" & BuildHeaderHint(sourceRange) & "）", requiredText))
            keyUniqueText = Trim(SafePrompt("主键唯一字段（逗号分隔，可空）（表头：" & BuildHeaderHint(sourceRange) & "）", keyUniqueText))
            phoneText = Trim(SafePrompt("电话校验字段（逗号分隔，可空）（表头：" & BuildHeaderHint(sourceRange) & "）", phoneText))
            dateText = Trim(SafePrompt("日期校验字段（逗号分隔，可空）（表头：" & BuildHeaderHint(sourceRange) & "）", dateText))
            errorText = Trim(SafePrompt("是否检查错误值（是/否，默认是）", errorText))
            ruleNameUsed = "手动输入"
        End If
    End If

    If ruleLoaded Then
        ruleSummary = "必填=" & requiredText & "；主键=" & keyUniqueText & "；电话=" & phoneText & "；日期=" & dateText
    ...
    If rulesJson = "{}" Then
        ...
        Exit Function
    End If
    ruleSummary = "必填=" & requiredText & "；主键唯一=" & keyUniqueText & "；电话=" & phoneText & "；日期=" & dateText & "；错误值=" & errorText
    ...
    SafeRecordWrite "excel_validate_report", "{""source"":""" & EscapeJson(sourceRange.Address) & """,""issueCount"":" & CStr(issueCount) & ",""rules"":""" & EscapeJson(rulesJson) & """,""ruleName"":""" & EscapeJson(ruleNameUsed) & """}", "office.excel.validatePack"
    ...
    saveBlob = "version=1" & vbCrLf & "kind=validate" & vbCrLf & "name=" & ruleNameUsed & vbCrLf & _
        "required=" & requiredText & vbCrLf & "keyUnique=" & keyUniqueText & vbCrLf & _
        "phone=" & phoneText & vbCrLf & "date=" & dateText & vbCrLf & "error=" & errorText
    MaybeSaveRule "validate", ruleNameUsed, saveBlob

    summary = "Excel 数据质检完成：问题明细=" & detailSheet.Name & "；质检报告=" & reportSheet.Name & _
        "；问题数=" & CStr(issueCount) & "；规则=" & ruleNameUsed & "；sourceUnchanged=true"
    Host.WriteClipboard summary
    SafeWriteLog summary
    HostExcelValidateReportPack = "{""ok"":true,""sourceUnchanged"":true,""commandId"":""excel.validate_report"",""detailSheet"":""" & EscapeJson(detailSheet.Name) & """,""reportSheet"":""" & EscapeJson(reportSheet.Name) & """,""issueCount"":" & CStr(issueCount) & ",""rule"":""" & EscapeJson(ruleNameUsed) & """,""writePlanPreview"":""" & EscapeJson(planPreview) & """,""message"":""" & EscapeJson(summary) & """}"
End Function

Function JoinNonEmpty(sep, a, b)
    Dim leftText, rightText
    leftText = Trim(CStr(a))
    rightText = Trim(CStr(b))
